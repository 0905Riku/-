...
#include "ui_recordgame.h"

short RecordGame::OFF_ONLINE = 0;
short RecordGame::ONLINE = 1;

int RecordGame::d = 80;
int RecordGame::choosePiece = -1;

RecordGame::RecordGame(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::RecordGame)
{
    ui->setupUi(this);
    ...
    connect(ui->lastStep,SIGNAL(clicked()),this,SLOT(lastStep()));
    connect(ui->nextStep,SIGNAL(clicked()),this,SLOT(nextStep()));
    connect(ui->GoTo,SIGNAL(clicked()),this,SLOT(GotoStep()));
...
void RecordGame::nextStep(){

    nextS();
}

void RecordGame::lastStep(){

    lastS();
}

void RecordGame::GotoStep(){
...
//关闭窗口
void RecordGame::closeEvent(QCloseEvent *event){
    emit closeSignal();
}


void RecordGame::initRecord(QByteArray& str){
...
//转化成棋盘上的点
ClickPoint RecordGame::toClickPoint(const QPoint& point){
    int x = point.x();
    int y = point.y();
    x -= d/2;
    y -= d/2;
...
void RecordGame::drawCross(int d,int x,int y,QPainter &painter){
    //画十字
    if(x != 1){
...
void RecordGame::paintEvent(QPaintEvent *event){
...
//初始化玩家信息
void RecordGame::initPlayer(const bool &gameMode, Player *p1, Player *p2){
...
QString RecordGame ::Translate(const ClickPoint& StartPoint,const ClickPoint& EndPoint,const int& start,const int& end){
    QString temp;
    int sx = StartPoint.x;
    int sy = StartPoint.y;
